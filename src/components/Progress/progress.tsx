...
export const Progress: FC<ProgressProps> = (props) => {
  const { percent, strokeHeight = 12, showText, style, animation } = props;
  ...
  if (circle) {
    return (
      ...
      >
        ...
        {showText && (
          <div className={classNames(`${prefixCls}-circle-text`)}>{`${text}${unit}`}</div>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="chocolate-progress" style={style}>
      <div className="bar-outer" style={{ height: `${strokeHeight}px` }}>
        <div
          className={classNames('bar-inner', {
            [`${prefixCls}-bar-inner-animate`]: animation,
          })}
          style={{ width: `${text}${unit}` }}
        >
          {showText && <span className="bar-inner-text">{`${text}${unit}`}</span>}
        </div>
      </div>
    </div>
  );
